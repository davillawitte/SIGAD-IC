...
@Component({
  ...
  templateUrl: './escalas-page.component.html',
  styleUrl: './escalas-page.component.scss',
})
export class EscalasPageComponent implements OnInit, OnDestroy {
  ...
  inlineActions(row: EscalaRow): InlineAction[] {
    ...
    const canMutate = this.canMutateSetor(row.setorId);
    if (
      canMutate &&
      this.auth.hasPermission('escalas.editar') &&
      (row.status === 'Rascunho' || row.status === 'Finalizada')
    ) {
      actions.push({ id: 'edit', label: 'Editar', icon: 'edit' });
    }
    if (canMutate && this.auth.hasPermission('escalas.publicar') && row.status === 'Finalizada') {
  ...
  menuActions(row: EscalaRow): InlineAction[] {
    ...
    if (
      canMutate &&
      this.auth.hasPermission('escalas.excluir') &&
      (row.status === 'Rascunho' || row.status === 'Finalizada')
    ) {
