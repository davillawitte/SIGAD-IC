...
import { EscalaMatrixViewComponent, MES_NOMES } from '../components/escala-matrix-view.component';
import { EscalasApiService } from '../services/escalas-api.service';
import type { EscalaDetail, StatusEscala } from '../models/escalas.models';
...
@Component({
  selector: 'app-escala-detail-page',
  imports: [
    CommonModule,
    MatDialogModule,
    PciAlertComponent,
  ...
  templateUrl: './escala-detail-page.component.html',
  styleUrl: './escala-detail-page.component.scss',
})
export class EscalaDetailPageComponent implements OnInit {
  ...
  canEdit(): boolean {
    return this.auth.hasPermission('escalas.editar') && this.canMutateSetor();
  }

  canPublish(): boolean {
    return this.auth.hasPermission('escalas.publicar') && this.canMutateSetor();
  }

  canFinalizar(): boolean {
    return this.auth.hasPermission('escalas.finalizar') && this.canMutateSetor();
  }

  canExcluir(): boolean {
    return this.auth.hasPermission('escalas.excluir') && this.canMutateSetor();
  }

  private canMutateSetor(): boolean {
    if (this.auth.isSuperAdmin()) return true;
    const setorId = this.escala()?.setorId;
  ...
  canSolicitarDevolucao(): boolean {
    ...
    return this.auth.hasPermission('escalas.solicitar_devolucao') && this.canMutateSetor();
  }
  ...
  canDevolverDireto(): boolean {
    ...
    return this.auth.isSuperAdmin() || this.canMutateSetor();
  }
