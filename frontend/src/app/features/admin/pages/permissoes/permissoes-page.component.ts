...
export class PermissoesPageComponent implements OnInit {
  ...
  readonly rows = signal<PermissaoRow[]>([]);

  readonly columns: PciColumn<PermissaoRow>[] = [
  ...
  goCreate(): void {
    void this.router.navigateByUrl('/permissoes/novo');
  }

  goEdit(row: PermissaoRow): void {
    void this.router.navigateByUrl(`/permissoes/editar/${row.id}`);
  }
  ...
  private reload(): void {
    ...
    this.api.listPermissoes({ page: this.page(), pageSize: this.pageSize() }).subscribe({
      next: (result) => {
        this.totalItems.set(result.totalItems);
        this.page.set(result.page);
        this.rows.set(
          result.items.map((p: PermissaoItem) => ({
            id: p.id,
            codigo: p.codigo,
            nome: p.nome,
            modulo: p.modulo,
      ...
      error: () => {
        this.error.set('Não foi possível carregar as permissões.');
        this.loading.set(false);
      },
    });
  }
